...
class ArchitectAgent(SRXBaseAgent):
    """
    ...
    def __init__(self, kernel_context, config: Dict[str, Any] = None):
        ...
        if not self.adapter and hasattr(kernel_context, "adapters"):
             ...
             # self.adapter = kernel_context.adapters.get("architect")
             pass
    ...
    def act(self, observation: str, context: Dict[str, Any] = None) -> str:
        ...
        """
        self.event_bus.publish("runtime", "agent.thinking", {"agent_id": self.id, "observation": observation[:100]})
