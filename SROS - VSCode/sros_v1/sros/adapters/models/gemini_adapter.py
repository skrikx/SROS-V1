...
class GeminiAdapter(ModelAdapter):
    """Logical Gemini adapter stub.

    This adapter routes requests to a provided client callable via config
    (config['client']) or degrades gracefully if not configured.
    """

    def initialize(self) -> bool:
        # Check for client in config or environment
        client = self.config.get('client') or os.environ.get('GEMINI_CLIENT')
    ...
    def get_metadata(self) -> Dict[str, Any]:
        return {
            'name': 'gemini',
            'model': self.config.get('model', os.environ.get('GEMINI_MODEL', 'gemini-2.0-flash')),
    ...
    def generate(self, prompt: str, tools: Optional[List[Dict]] = None, context: Optional[Dict[str, Any]] = None, stream: bool = False, **kwargs) -> AdapterResult:
        ...
        if callable(client):
            try:
                ...
                logger.exception('Gemini client raised')
                return AdapterResult(success=False, data=None, error=str(e))

        return AdapterResult(success=False, data=None, error='No Gemini client configured')
