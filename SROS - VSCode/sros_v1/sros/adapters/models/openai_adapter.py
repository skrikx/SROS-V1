...
class OpenAIAdapter(ModelAdapter):
    """Logical OpenAI adapter stub.

    Routes to configured client callable (`config['client']`) if present.
    """

    def initialize(self) -> bool:
        client = self.config.get('client') or os.environ.get('OPENAI_CLIENT')
    ...
    def get_metadata(self) -> Dict[str, Any]:
        return {
            'name': 'openai',
            'model': self.config.get('model', os.environ.get('OPENAI_MODEL', 'gpt-4.1-mini')),
    ...
    def generate(self, prompt: str, tools: Optional[List[Dict]] = None, context: Optional[Dict[str, Any]] = None, stream: bool = False, **kwargs) -> AdapterResult:
        ...
        if callable(client):
            try:
                ...
                logger.exception('OpenAI client raised')
                return AdapterResult(success=False, data=None, error=str(e))

        return AdapterResult(success=False, data=None, error='No OpenAI client configured')
