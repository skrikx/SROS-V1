...
class SRXMLParser:
    ...
    def parse_to_object(self, file_path: str) -> Union['SRXAgent', 'SR8Workflow', 'GovernancePolicy']:
        ...
        if tag == 'srx_agent_prompt':
            ...
            identity_data = data.get('identity', {})
            # Steps are at root level, not nested in workflow element
            ...
            for step in steps_data:
                # Extract instruction from input element or #text
                instruction = step.get('#text', '')
                if not instruction and 'input' in step:
                    input_elem = step.get('input', {})
                    instruction = input_elem.get('#text', '') if isinstance(input_elem, dict) else str(input_elem)
