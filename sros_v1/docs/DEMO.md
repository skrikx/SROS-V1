...
- The repository root is `sros_v1`
- A valid Gemini API key is stored in `.env` (`GEMINI_API_KEY=YOUR_KEY`)
...
3. **Run the Architect agent**
   ```bash
   python -m sros.nexus.cli.main agent run architect "Analyze: The kernel event bus is experiencing message delivery delays."
   ```
...
4. **Store the analysis in short‑term memory**
   ```bash
   python -m sros.nexus.cli.main memory write "$(cat analysis.txt)" --layer short
   ```
   The result is kept for the current session.

5. **Run the Builder agent**
   ```bash
   python -m sros.nexus.cli.main agent run builder "Create a new adapter for a custom HTTP tool"
   ```
...
6. **Persist the generated code in long‑term memory**
   ```bash
   python -m sros.nexus.cli.main memory write "$(cat adapter.py)" --layer long --key "custom_http_adapter"
   ```

7. **Run the Tester agent**
   ```bash
   python -m sros.nexus.cli.main agent run tester "Generate pytest tests for the new adapter"
   ```
